...
DRAWPADDLE:
    ...
    FOR1_DRAWPADDLE:
        ...
        add $t4 $t2 $t4 #Endereco em vetor unidimensional(paddley+i)
        sll $t4 $t4 2 #Endereco * 4, tranformando para endereco de 32bits
        sw $t6 0($t4) #screen[paddlex][paddley + i] = '|'

        sub $t5 $a1 $t0 #paddley - i
        add $t5 $t2 $t5 #Endereco em vetor unidimensional(paddley-i)
...
MOVEPADDLEONE:
    ...
    addi $t0 $s2 3 #paddleOney + PADDLE_RADIUS
    ...
    subi $t0 $s2 3 #paddleOney - PADDLE_RADIUS
...
MOVEBALL:
    ...
    beq $t1 $zero END_IF1_MOVEBALL #if (ballx > WIDTH - MARGIN_HORIZONTAL)
        addi $t2 $s4 -3 #paddleTwoy - PADDLE_RADIUS
        addi $t3 $s4 3 #paddleTwoy + PADDLE_RADIUS
    ...
    beq $t0 $zero END_IF3_MOVEBALL #if (ballx < MARGIN_HORIZONTAL)
        addi $t1 $s2 -3 #paddleOney - PADDLE_RADIUS
        addi $t2 $s4 3 #paddleOney + PADDLE_RADIUS
